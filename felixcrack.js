...
┏━━━━━━━━━━━━━━━━━
┠⊷️ *Suscríbete*:
┠⊷ * https://youtube.com/channel/UCMejWoDRfa3DpDybSQvIwMQ
...
const faketokoforwaded = (teks) => {
    ...
    const sendButVideo = async (
      ...
      kma = vid1;
      mhan = await FxBot.prepareMessage(from, kma, video);
      const buttonMessages = {
...
if (!isGroup) return reply(mess.only.group)
                 ...
                 case prefix+ 'pm':
				    ...
				    FxBot.sendMessage(from, yeh, text, {quoted: mel })			       
					break
case prefix+ 'leave':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isGroupAdmins) return reply(mess.only.admin)
...
console.log('\x1b[1;37m>', '[', '\x1b[1;32m EXEC \x1b[1;37m', ']', time, color(">", "green"), 'from', color(pushname), 'args :', color(args.length))
} catch(e){
reply(String(e))
}
}     
